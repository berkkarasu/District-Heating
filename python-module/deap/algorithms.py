...
def varPop(population, toolbox, lambda_):
    ...
    num_offspring_with_cx = lambda_ // 2
    offspring = []
    for _ in range(num_offspring_with_cx):
        ind1 = toolbox.clone(
            random.choice(
                sorted_population[: int(len(sorted_population) * 0.2)]
            )
        )
        ind2 = toolbox.clone(
            random.choice(
    ...
    for _ in range(lambda_ - num_offspring_with_cx):
        ind = toolbox.clone(
            random.choice(
                sorted_population[: int(len(sorted_population) * 0.6)]
            )
        )
        ind, = toolbox.mutate(ind)
...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    # Begin the generational process
    for gen in tqdm(range(1, ngen + 1), ncols = 100):
        # Vary the population
        offspring = varPop(population, toolbox, lambda_) # varOr(population, toolbox, lambda_, cxpb, mutpb)
